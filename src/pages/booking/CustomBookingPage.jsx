...
const CustomBookingPage = () => {
  ...
  const filteredBookings = useSelector(selectFilteredCustomBookings);

  const { custombookings, isLoading, isError, message } = useSelector(
    (state) => state.booking
  );
  useEffect(() => {
    dispatch(RESETCUSTOMBOOKINGARR());
    dispatch(getSingleCustomBooking());
  ...
  const cancelbookingPack = async (id) => {
    await dispatch(CancelCustomBooking(id));
    await dispatch(getSingleCustomBooking());
  };
  ...
  return (
    <div className="p-1 w-full overflow-x-auto">
      <div className="flex justify-between items-center ">
        <span>
          <h3 className="md:text-2xl">Custom Bookings List</h3>
        </span>
        <span>
